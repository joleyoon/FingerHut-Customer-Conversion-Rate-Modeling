...
ARTIFACT_DIR = PROJECT_ROOT / "results" / "experiments" / "rnn"
...
MODEL_PATH = ARTIFACT_DIR / "rnn_gru_model.pt"
...
def train_rnn(train_arrays: dict, vocab: dict[str, int]) -> JourneyRNN:
    ...
    device = get_device()
    model = JourneyRNN(n_events=len(vocab), n_static=train_arrays["static"].shape[1]).to(device)
...
def write_submission(model: JourneyRNN, test_arrays: dict) -> pd.DataFrame:
    device = get_device()
...
def main() -> pd.DataFrame:
    ...
    model = train_rnn(train_arrays, vocab)
